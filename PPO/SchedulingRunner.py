...
from PPOTrainer import Trainer
...
class Runner:
    def __init__(
        ...
        self.epoch = 1
        self.step_scaler = 1000 // (
                self.runner_params['training']['episode'] * self.runner_params['training']['K_epochs'])
    ...
    def train(self):
        ...
        for self.epoch in range(self.epoch, total_epochs + 1):
            (score_t, _, _, _, loss_t, model_param) = self.trainer.train(self.env, num_epoch=self.epoch)
    ...
    def _log_training_progress(self, score_t, loss_t, model_param):
        ...
        if (self.epoch % (self.validation_freq * self.step_scaler) == 0) or self.epoch == 1:
